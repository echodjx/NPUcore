...
int ssysfile_open(const char *path, uint32_t open_flags);        // Open or create a file. FLAGS/MODE per the syscall.
int ssysfile_close(int fd);                                      // Close a vnode opened  
int ssysfile_read(int fd, void *base, size_t len);               // Read file
int sysfile_write(int fd, void *base, size_t len);              // Write file
int sysfile_seek(int fd, off_t pos, int whence);                // Seek file  
int ssysfile_seek(int fd, off_t pos, int whence);                // Seek file  
