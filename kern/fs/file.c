...
static void
sfd_array_free(struct file *file) {
    assert(file->status == FD_INIT || file->status == FD_CLOSED);
    assert(fopen_count(file) == 0);
    if (file->status == FD_CLOSED) {
        vfs_close(file->node);
...
void
sfd_array_open(struct file *file) {
    assert(file->status == FD_INIT && file->nodes!= NULL);
    file->status = FD_OPENED;
    fopen_count_inc(file);
}
...
void
sfd_array_close(struct file *file) {
    assert(file->status == FD_OPENED);
    assert(fopen_count(file) > 0);
    file->status = FD_CLOSED;
    if (fopen_count_dec(file) == 0) {
...
file_open(char *path, uint32_t open_flags) {
    ...
    case 1: readable = 1; break;
    case 2: writable = 1; break;
    case 3:
        readable = writable = 1;
        break;
    default:
...
int
sfile_open(char *path, uint32_t open_flags) {
    bool readable = 0, writable = 0;
    switch (open_flags & O_ACCMODE) {
    case 0: readable = 1; break;
...
int
sfile_read(int fd, void *base, size_t len, size_t *copied_store) {
    int ret;
    struct file *file;
    *copied_store = 0;
    if ((ret = fd2file(fd, &file)) != 0) {
...
file_write(int fd, void *base, size_t len, size_t *copied_store) {
    ...
    *copied_store = 0;
    ...
    if (ret != FR_OK) {
        cprintf("Write %s err[%d]\n", ret);
    } else {
        cprintf("Write %d bytes to %s ok\n", v_ret_len);
    }
    *copied_store=v_ret_len;
    fd_array_release(file);
    return ret;
...
int
sfile_seek(int fd, off_t pos, int whence) {
    struct stat __stat, *stat = &__stat;
    int ret;
    struct file *file;
    if ((ret = fd2file(fd, &file)) != 0) {
    ...
    case LSEEK_END:
        if ((ret = vop_fstat(file->nodes, stat)) == 0) {
            pos += stat->st_size;
        }
        break;
    default: ret = -E_INVAL;
    ...
    if (ret == 0) {
        if ((ret = vop_tryseek(file->nodes, pos)) == 0) {
            file->pos = pos;
        }
...
file_fstat(int fd, struct stat *stat) {
    ...
    fd_array_acquire(file);
    FILINFO *fstat=kmalloc(sizeof(FILINFO));
