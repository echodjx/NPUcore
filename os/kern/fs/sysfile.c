...
/* sysfile_open - open file */
int
ssysfile_open(const char *__path, uint32_t open_flags) {
    int ret;
    char *path;
    if ((ret = copy_path(&path, __path)) != 0) {
        return ret;
    }
    ret = sfile_open(path, open_flags);
    kfree(path);
    return ret;
}
...
int
ssysfile_close(int fd) {
    return sfile_close(fd);
}
...
int
ssysfile_read(int fd, void *base, size_t len) {
    struct mm_struct *mm = current->mm;
    if (len == 0) {
        return 0;
    }
    ...
    while (len != 0) {
        ...
        ret = sfile_read(fd, buffer, alen, &alen);
        if (alen != 0) {
            lock_mm(mm);
            {
                if (copy_to_user(mm, base, buffer, alen)) {
...
int
ssysfile_seek(int fd, off_t pos, int whence) {
    return sfile_seek(fd, pos, whence);
}
