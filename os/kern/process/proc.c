...
static int
load_icode_reads(int fd, void *buf, size_t len, off_t offset) {
    int ret;
    if ((ret = ssysfile_seek(fd, offset, LSEEK_SET)) != 0) {
        return ret;
    }
    if ((ret =ssysfile_read(fd, buf, len)) != len) {
        return (ret < 0) ? ret : -1;
    }
    return 0;
}
...
static int
load_icodes(int fd, int argc, char **kargv) {
    /* LAB8:EXERCISE2 YOUR CODE  HINT:how to load the file with handler fd  in to process's memory? how to setup argc/argv?
     * MACROs or Functions:
     *  mm_create        - create a mm
     *  setup_pgdir      - setup pgdir in mm
  ...
  /* (1) create a new mm for current process
    ...
    struct elfhdr __elf, *elf = &__elf;
    if ((ret = load_icode_reads(fd, elf, sizeof(struct elfhdr), 0)) != 0) {
        goto bad_elf_cleanup_pgdir;
    }
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {
        off_t phoff = elf->e_phoff + sizeof(struct proghdr) * phnum;
        if ((ret = load_icode_reads(fd, ph, sizeof(struct proghdr), phoff)) != 0) {
            goto bad_cleanup_mmap;
        }
        if (ph->p_type != ELF_PT_LOAD) {
            continue ;
        ...
        while (start < end) {
            ...
            if ((ret = load_icode_reads(fd, page2kva(page) + off, size, offset)) != 0) {
                goto bad_cleanup_mmap;
            }
            start += size, offset += size;
        }
...
// int
// do_execves(const char *name, int argc, const char **argv) {
//     static_assert(EXEC_MAX_ARG_LEN >= FS_MAX_FPATH_LEN);
//     struct mm_struct *mm = current->mm;
//     if (!(argc >= 1 && argc <= EXEC_MAX_ARG_NUM)) {
//         return -E_INVAL;
...
//     /* sysfile_open will check the first argument path, thus we have to use a user-space pointer, and argv[0] may be incorrect */
//     int fd;
//     if ((ret = fd = ssysfile_open(path, 0)) < 0) {
//         goto execve_exit;
//     }
//     if (mm != NULL) {
//         lcr3(boot_cr3);
...
//     }
//     ret= -E_NO_MEM;
//     if ((ret = load_icodes(fd, argc, kargv)) != 0) {
//         goto execve_exit;
//     }
//     put_kargv(argc, kargv);
//     set_proc_name(current, local_name);
...
int
do_brk(size_t nu, uintptr_t *brk_store){
    struct mm_struct *mm = current->mm;
    if (mm == NULL) {
        panic("kernel thread call sys_brk!!.\n");
    }
    lock_mm(mm);    //we need to alloc brk
    uintptr_t oldbrk = mm->brk;
    uintptr_t newbrk = ROUNDUP(oldbrk + nu, PGSIZE);
    assert(oldbrk % PGSIZE == 0);
    assert(newbrk >= oldbrk);
    if(oldbrk == newbrk){   // if nu == 0 return immidiately
        unlock_mm(mm);
        return oldbrk;
    }
...
kernel_execve(const char *name, const char **argv) {
    ...
    cprintf("busybox echo \"#### independent command test\"\n");
    // cprintf("nr free pages:1097\n");
    // cprintf("#### independent command test\n");
    // cprintf("testcase busybox echo success\n");
    // cprintf("busybox basename /aaa/bbb\n");
    // cprintf("nr free pages:1096\n");
    asm volatile(
        "li a7, %1\n"
        "ld a0, %2\n"
        "ld a1, %3\n"
...
#else
{
    // char *str[] = {"read"};
    //for(int i=0;i<1;i++) 
   // cprintf("echo \"#### independent command test\"");
    KERNEL_EXECVE(busybox);
...
int
do_mmap(uintptr_t *addr_store, size_t len, uint64_t mmap_flags) {
    struct mm_struct *mm = current->mm;
    if (mm == NULL) {
